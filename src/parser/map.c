...
static void increase_size(unsigned short new_size[2]) {
  if (new_size[0] < data.map.size[0])
    new_size[0] = data.map.size[0];
  if (new_size[1] < data.map.size[1])
    new_size[1] = data.map.size[1];
  if (new_size[0] == data.map.size[0] && new_size[1] == data.map.size[1])
    return;
...
void map(char *line) {
  ...
  for (unsigned short x = len; x < data.map.size[0]; x++)
    map_line[x] = 0;
