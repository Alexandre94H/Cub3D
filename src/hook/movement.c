...
static bool can_move(bool horizontal, float update) {
  ...
  if (data.map.data[(int)position[1] * data.map.size[0] + (int)position[0]] !=
      0)
    return false;
...
void move(const float movement[2]) {
  float update[2] = {
      data.player.direction[0] * movement[0] +
          data.player.direction[1] * movement[1],

      data.player.direction[1] * movement[0] -
          data.player.direction[0] * movement[1],
  };
...
void rotate(float rotation) {
  ...
  data.player.direction[0] =
      direction[0] * cosf(rotation) - direction[1] * sinf(rotation);
  data.player.direction[1] =
      direction[0] * sinf(rotation) + direction[1] * cosf(rotation);
}
