	...
	t_resolution position)
{
	...
	if (is_floor)
		color = runtime->file.floor.data[runtime->file.floor.resolution.width
			* (unsigned short)(runtime->file.floor.resolution.height * floor.y)
			+ (unsigned short)(runtime->file.floor.resolution.width * floor.x)];
	else
		color = runtime->file.ceil.data[runtime->file.ceil.resolution.width
			* (unsigned short)(runtime->file.ceil.resolution.height * floor.y)
			+ (unsigned short)(runtime->file.ceil.resolution.width * floor.x)];
	...
	t_position *step)
{
	...
	if (y > runtime->file.resolution.height / 2 + runtime->player.pitch * 200)
		row_distance = (0.5 * runtime->file.resolution.height
			+ runtime->player.position.z * 200) /
			(float)(y - runtime->file.resolution.height / 2
			- runtime->player.pitch * 200);
	else
		row_distance = (0.5 * runtime->file.resolution.height
		- runtime->player.position.z * 200) /
		(float)(runtime->file.resolution.height / 2
		- y + runtime->player.pitch * 200);
...
void		floor_ajust(t_position *floor)
{
	floor->x -= (int)floor->x;
	floor->y -= (int)floor->y;
	if (floor->x < 0)
